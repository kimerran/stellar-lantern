...
// MV3 can tear this worker down at any time — if so, `session` is gone and the
// user must re-unlock (we answer signing with code: 'LOCKED').
...
function toErrorResult(err: unknown): Result<never> {
  ...
  // Horizon submission error → surface a readable reason (SPEC §6.4).
...
chrome.runtime.onMessage.addListener((req: Request, _sender, sendResponse) => {
  handle(req)
    .then(sendResponse)
    .catch((err) => sendResponse(toErrorResult(err)));
